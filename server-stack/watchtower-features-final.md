...
Ağ Trafiği
Kullanıcı başına dosya sunucusundan günlük çekilen veri miktarı ve aktarım hızı
Şirket içi sunucular arasında yan hareket trafik pateni — hedef host çeşitliliği ve bağlantı sayısı
Cihaz başına DNS sorgu sayısı, yüksek entropi subdomain tespiti ve şüpheli sorgu tipi dağılımı
İç ağda dosya paylaşım ve kimlik doğrulama protokolünün güvenli sürümden zafiyetli sürüme zorla düşürülmesi
Onaysız DHCP sunucusu tespiti, ağ adresi sahteciliği ve aynı IP'nin çift kiralanması
Başarısız kimlik doğrulama denemesi patlamasının ardından başarılı erişim — kaba kuvvet veya parola püskürtme
★ İç ağda port taraması veya servis keşif davranışı — lateral hareketten önce gelen keşif adımı
Kimlik ve Erişim
Saatlik oturum açma sayısı, hedef sistem çeşitliliği ve gece saatlerindeki yoğunlaşma
Aynı kullanıcının fiziksel olarak ulaşamayacağı farklı iç konumlardan eş zamanlı oturum açması
Yalnızca uygulamalar için tanımlanmış servis hesabının insan tarafından masaüstü veya uzak masaüstü oturumu açmak için kullanılması
Etki alanı yöneticileri ve ayrıcalıklı gruplar gibi hassas gruplara yapılan onaysız üye ekleme ve çıkarma işlemleri
Her uygulama anahtarının hangi kaynak adreslerden kaç çağrı yaptığı, birden fazla adresten kullanım ve iptal sonrası yeniden devreye girme
Gizli bilgi deposundaki okuma sayısının kısa sürede olağandışı biçimde artması ve bakım penceresi dışındaki erişimler
Yönetici tarafından yapılan parola sıfırlama ve hesap kilit açma işlemlerinin helpdesk talebiyle örtüşmesi
Sunucular arası zincirleme uzak masaüstü veya uzak komut yürütme oturumu atlaması
Mail ve İletişim
Günlük gönderilen e-posta sayısı, ek boyutu ve dağıtım listesi alıcı sayısındaki sapma
Posta kutusuna yeni otomatik yönlendirme, temsilci veya filtre kuralı eklenmesi
Kullanıcının kendi posta kutusu dışındaki paylaşımlı veya delege edilmiş posta kutularına erişimi
E-posta ekinin uzantısı ile gerçek dosya türünün örtüşmemesi ve şifreli arşiv kullanımı
E-postalara eklenen gizli alıcı sayısı ve beklenmedik dağıtım genişlemesi
★ Şirket e-posta adresinden gmail, hotmail veya yahoo gibi kişisel adreslere ek içeren posta gönderimi
★ Bilinmeyen veya rakip olabilecek harici kuruluşlara ek içeren posta gönderimi
★ E-posta konu satırı veya gövdesinde maaş, istifa, gizli, dava, rakip firma adı gibi hassas anahtar kelime tespiti
★ Yıllarca dokunulmayan eski posta arşivinin kısa sürede toplu olarak okunması
★ Kişi listesinin veya adres defterinin dışa aktarılması
Dosya Sistemi
Kullanıcının kendi departmanı dışındaki paylaşım klasörlerine erişim sayısı ve ilk kez erişilen hassas dizinler
Belirli bir sürede okunan ve yazılan dosya sayısı oranındaki sapma — toplu okuma baskınlığı
Ağ sürücüsünde çok kısa sürede çok sayıda dosyanın adının veya uzantısının değiştirilmesi
Kullanıcının içerik açmadan yalnızca dosya adlarını listeleyerek hassas dizinleri dolaşması
Gizli paylaşım klasörünün erişim listesine herkese açık okuma veya yazma yetkisi eklenmesi
Kullanıcının kişisel bilgisayarında hassas belge türlerinin kısa sürede olağandışı biçimde birikmesi
★ Gizli veya dahili olarak etiketlenmiş dosyanın kısıtlanmamış bir alana kopyalanması veya taşınması
★ İç bilgi bankası, wiki veya intranet portalından alışılmadık sayıda sayfa veya dosya indirilmesi
Uygulama ve Süreç
Kullanıcı başına günlük dönen veritabanı satır ve veri miktarı ile erişilen tablo çeşitliliği
Kullanıcının hangi uygulamaları ne kadar süre kullandığı ve bu uygulamaların altında başlatılan şüpheli alt süreçler
Kullanıcı veya servis başına iç uygulama arayüzü çağrı sıklığı ve ilk kez erişilen uç noktalar
Kullanıcının iç uygulama uç noktalarından aldığı yetkisiz erişim hatalarının toplam istek içindeki oranı
Kullanıcının kaynak kod deposundan klonladığı proje sayısı ve toplam indirilen veri miktarı
Güvenlik izleme sistemine yeni kural eklenmesi, kural devre dışı bırakılması veya incelenmeksizin toplu uyarı kapatma
Sanallaştırma yönetim konsoluna kişisel hesapla veya yetkisiz şekilde doğrudan erişim
★ Kullanıcının makinesinde yeni zamanlanmış görev veya sistem servisi oluşturulması — kalıcılık mekanizması
★ Yedekleme gölge kopyalarının silinmesi veya devre dışı bırakılması — şifreleme öncesi en erken fidye yazılımı sinyali
★ Kodlanmış veya yürütme politikasını atlayan betik komutlarının çalıştırılması
Donanım ve Uç Nokta
Harici depolama cihazının bağlanmasının ardından kısa sürede aktarılan veri miktarı ve dosya türleri
Kullanıcı başına baskı sayısı ve hassas belge kuyruğuna gönderilen içerik
Kullanıcı makinesindeki ağ bağdaştırıcısının tüm trafiği yakalayan izleme moduna alınması
Uç nokta veri koruma ajanının mesai saatlerinde çevrimdışı kalma süresi ve devre dışı bırakılma girişimi
Davranışsal
Panoya kopyalanan verinin boyutu, kaynak uygulama ve hedef uygulama arasındaki akış
Hassas uygulama ekranındayken alınan ekran görüntüsü sayısı ve frekansı
Kullanıcının rolü için normal kabul edilen sunucu kümesinin dışına çıkması
Kullanıcının güvenlik platformunda maaş, parola, işten çıkarma gibi hassas terimlerle yaptığı sorgular
★ Daha önce hiç kaydedilmemiş donanım kimliğinden gelen oturum açma denemesi
★ Haftalar içinde dağılan normal aktivite örüntüsünün saatler içinde aşırı biçimde yoğunlaşması
★ Uzun süre hiç kullanılmayan veya hiç girilmemiş sistemlere aniden erişim
★ Aynı departman ve rolde çalışan kişiler grubuyla kıyaslandığında istatistiksel olarak belirgin sapma
★ Dış Bağlantı ve Çıkış Trafiği (yeni kategori)
★ Kurumsal ağdan kişisel bulut depolama hizmetlerine büyük dosya yükleme
★ Kurumsal ağdan daha önce hiç bağlanılmamış harici adreslere yüksek hacimli veri transferi
★ Proxy üzerinden şifreli tünel veya alışılmadık protokol kullanımı
Korelasyon
Fiziksel bina giriş kaydı olmaksızın dahili ağda aktif görünen kullanıcı
Tek kullanıcının kısa zaman penceresinde birden fazla riskli sinyali bir arada üretmesi
İnsan kaynakları sistemine işten ayrılma kaydı düşüldükten sonra hesabın aktif kalmaya devam etmesi
Aynı belgeye kısa aralıkla farklı departmanlardan birden fazla kullanıcının erişmesi
Gece saatinde hem fiziksel bina girişi hem de yüksek hacimli veri aktarımının eş zamanlı gerçekleşmesi
★ İnsan Kaynakları ve Personel Yaşam Döngüsü (yeni kategori)
★ Yeni işe başlayan çalışanın ilk günlerde çok sayıda farklı sisteme alışılmadık hızda erişmesi
★ Rol değişikliği veya terfi sonrasında kaldırılmamış eski yetkilerle erişimin sürmesi
★ Resmi izin kaydı olan kullanıcının sistem veya ağ aktivitesine devam etmesi
★ Üçüncü taraf danışman veya yüklenicinin sözleşme kapsamı dışında sistem kullanımı
Zamanlama
Kullanıcının yıllarca sürdürdüğü mesai saati düzeninin dışında sisteme bağlanması
Saatlerce boşta kalan oturumun ardından hassas alanlarda ani aktivitenin başlaması
Kullanıcının kendi departmanının çalışma ritmiyle örtüşmeyen saatlerde yoğun aktivite sergilemesi


Mail ve İletişim (güncellenmiş — yeni eklemeler)
...mevcut 25 sinyal devam eder...

★ Kullanıcının normal yazışma üslubundan belirgin sapma — resmiyet seviyesi, dil tonu veya yazım kalıplarındaki ani değişim — hesap ele geçirilme veya sosyal mühendislik sinyali
★ Şirket iletişim politikasıyla bağdaşmayan, tehdit içeren, ayrımcı veya gizlilik ihlali riski taşıyan yasaklı ifadelerin e-postalarda kullanımı
★ Daha önce hiç yazışılmamış harici kişilere veya şirket çalışanı olmayan alıcılara hassas konu içeren posta gönderimi
★ Şirket posta altyapısı dışında kişisel veya bilinmeyen başka bir hesapla kurumsal posta sistemine giriş denemesi
★ Yapay Zeka Kullanımı (yeni kategori)
★ Şirket gizli verilerinin ChatGPT, Claude, Gemini veya benzeri harici yapay zeka platformlarına gönderilmesi — müşteri bilgisi, finansal veri, personel kaydı, hukuki belge
★ Kaynak kod veya sistem mimarisi içeriğinin yapay zeka sohbet arayüzüne yapıştırılması — dahili servis adları, bağlantı bilgileri, iş mantığı
★ Yapay zeka platformuna iletilen sorgularda şirket içi sistem adı, kullanıcı adı, parola ipucu veya ağ adresi geçmesi
★ Onaylanmamış veya şirket politikasında tanımlanmamış yapay zeka araçlarının kurumsal ağ üzerinden kullanımı
★ Yapay zeka platformuna yüklenen dosyaların boyutu ve içerik türü — toplu belge yükleme, veri seti aktarımı
★ Yapay zeka aracılığıyla şirket süreçleri, yetkiler veya güvenlik prosedürleri hakkında detaylı bilgi sorgulanması — içeriden birinin sistemi tanımaya çalışması veya sosyal mühendislik hazırlığı
★ Çalışanın yapay zeka konuşma geçmişinde şirket rakiplerine, stratejik planlara veya devam eden hukuki süreçlere ilişkin içerik
